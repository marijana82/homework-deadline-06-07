...
function tvNameFull (tvString) {
let fullNameTv = 0;
for (let i = 0; i < tvString.length; i++) {
    fullNameTv = tvString[i].brand + tvString[i].type + "-" + tvString[i].name;
    }
    return (tvString);
}
console.log(tvNameFull ("brand" + "type " + "- name"));




function priceInEuros(priceFormat) {
    let priceTv = 0;
    for (let i = 0; i < priceFormat.length; i++) {
    }
    return (priceFormat);
}
console.log(priceInEuros(379));
